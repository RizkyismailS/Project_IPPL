                    ...
                    <div class="card mb-3">
                        <div class="card-body">
                            ...
                            <?php if ($sesi['status_final'] === 'ABSEN_SEKARANG'): ?>
                                <form action="<?= base_url('mahasiswa/submitAbsensi') ?>" method="post" enctype="multipart/form-data">
                                    <?= csrf_field() ?>
                                    <input type="hidden" name="id_sesi" value="<?= esc($sesi['id_sesi']) ?>">
                                    <div class="mb-2">
                                        <label for="status_absen_<?= $sesi['id_sesi'] ?>" class="form-label">Status Kehadiran</label>
                                        <select name="status_absen" id="status_absen_<?= $sesi['id_sesi'] ?>" class="form-select" required>
                                    ...
                                    <div class="mb-2">
                                        <label for="keterangan_<?= $sesi['id_sesi'] ?>" class="form-label">Keterangan (opsional)</label>
                                        <input type="text" name="keterangan" id="keterangan_<?= $sesi['id_sesi'] ?>" class="form-control">
                                    </div>
                                    <?php if ($sesi['perlu_bukti_foto']): ?>
                                    <div class="mb-2">
                                        <label for="bukti_foto_<?= $sesi['id_sesi'] ?>" class="form-label">Bukti Foto (wajib)</label>
                                        <input type="file" name="bukti_foto" id="bukti_foto_<?= $sesi['id_sesi'] ?>" class="form-control" accept="image/*" required>
                                    </div>
                                    <?php endif; ?>
